...
public class SalvarProdutoServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id == -1) {
            sucesso = produtoDAO.inserir(descricao, preco, null, quantidade, categoriaId);
        } else {
            sucesso = produtoDAO.atualizar(descricao, preco, null, quantidade, categoriaId, id);
        }
        // saída
        if (sucesso) {
        ...
        RequestDispatcher rd = request.getRequestDispatcher("/admin/ListarProdutos?descricao=");
